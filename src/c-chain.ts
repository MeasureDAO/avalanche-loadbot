...
export async function exec(params: LoadBotParams) {
  ...
  const runValueTx = async (rate: number, nonce?: number) => {
    ...
    for (let i = 0; i < rate; i++) {
      sendTxn(web3, evm, nonce + i, recvAccount.address)
        .then((val) => {
          pushResult(val);
          runInAction(() => {
            const val = finishedCount.get() + 1;
...
Total loadbot execution time    = ${time[0] + time[1] * SEC_PRE_NS}s
      `);
...
async function getFees(evm: EVMAPI) {
  const baseFee = parseInt(await evm.getBaseFee(), 16);
  let maxPriorityFeePerGas = parseInt(
      await evm.getMaxPriorityFeePerGas(),
      16
  );
