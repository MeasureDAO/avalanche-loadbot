...
export async function cChainExec() {
  ...
  web3.eth.accounts.wallet.add(recvAccount);

  web3.eth.defaultAccount = EVMAddr;

  web3.eth.accounts.wallet.add(EVMPrivKey);
  let waiting: ora.Ora;
  ...
  const pushResult = (val: TransactionReceipt) => {
    runInAction(() => {
      // Here we push the blocknumber. don't worry about the flow as we just need
      // to get the first and last blockNum.
  ...
  const runValueTx = async (nonce?: number) => {
    ...
    for (let i = 0; i < lParams.rate; i++) {
      let { maxFeePerGas, maxPriorityFeePerGas } = await getFees();
  ...
  const exec = async (nonce?: number) => {
    switch (mode) {
      case 'plain':
        await runValueTx(nonce);
        break;
      case 'contract':
        await runContractTx(nonce);
        break;
    }
  };
